...
class neuralNetwork:
    ...
    def train(self,inputs_list,targets_list):
        ...
        # self.who += (self.lr * numpy.dot(output_errors,numpy.transpose(hidden_outputs)))/batch_size+lamda/batch_size*self.who
        ...
        # self.bho=self.bho+output_errors/batch_size
        # self.bih=self.bih+deltabih/batch_size

        #注意，这里的w和b的更新用的是-=！之前用的是+=！
        output_errors = final_outputs-targets
        dwho=self.lr *(( numpy.dot(output_errors,numpy.transpose(hidden_outputs)))/batch_size+lamda/batch_size*self.who)
        self.who-=dwho
        self.bho-=self.lr *(numpy.sum(output_errors,axis=1,keepdims=True)/batch_size)
        dZ1=numpy.dot(self.who.T,output_errors)*hidden_outputs * (1.0 - hidden_outputs)
        dwih=self.lr *(numpy.dot(dZ1,numpy.transpose (inputs))+lamda/batch_size*self.wih)
