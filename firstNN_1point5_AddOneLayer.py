"""
使用python深度学习的数
在firstNeuralNetwork的基础上多添加了一个隐藏层
"""
...
class neuralNetwork:
    ...
    def train(self,inputs_list,targets_list):
        ...
        h2input=numpy.dot(self.whh,hidden_outputs)
        h2output=self.activation_function(h2input)



        # calculate signals into final output layer
        final_inputs = numpy.dot(self.who, h2output)
        ...
        hiderr1=numpy.dot(self.whh.T,hidden_errors)

        self.who += self.lr * numpy.dot((output_errors *
                                         final_outputs * (1.0 - final_outputs)),
                                        numpy.transpose(h2output))

        self.whh += self.lr * numpy.dot((hidden_errors *
                                         h2output * (1.0 - h2output)),
                                        numpy.transpose(hidden_outputs))

        # update the weights for the links between the input and hidden layers
        self.wih += self.lr * numpy.dot((hiderr1*
