"""
在firstNN_3_realMBGD的基础上加入lamda
"""
import numpy
...
class neuralNetwork:
    ...
    def train(self,inputs_list,targets_list):
        ...
        #这个式子只是用来求误差的啊！！！实际上的反向导数是不用加上这个的啊！！！
        #output_errors = targets - final_outputs    不是一个代价函数啊！！！ 是代价函数对于a的偏导数的结果来的啊！！！！
        #反向传播的时候误差的求解根本不需要这个 的啊！！！
        # output_errors+= lamda*(numpy.sum(numpy.square(self.wih)) + numpy.sum(numpy.square(self.who)))/batch_size/2
        hidden_errors = numpy.dot(self.who.T, output_errors)

        # print("----------------------原来的 who----------------------------")
        # print(self.who[0])
        # print("----------------------delta who----------------------------")
        # print( ((self.lr * numpy.dot((output_errors *
        #                               final_outputs * (1.0 - final_outputs)),
        #                              numpy.transpose(hidden_outputs)))/batch_size)[0])
        # print("----------------------lamda式子 who----------------------------")
        # print((lamda/batch_size*self.who)[0])
        # print("----------------------lamda 和 batch_size who----------------------------")
        # print(lamda,batch_size,lamda*(numpy.sum(numpy.square(self.wih)) + numpy.sum(numpy.square(self.who)))/batch_size/2)
        #

        self.who += (self.lr * numpy.dot((output_errors *
                                          final_outputs * (1.0 - final_outputs)),
                                         numpy.transpose(hidden_outputs)))/batch_size+lamda/batch_size*self.who
        self.wih += (self.lr * numpy.dot((hidden_errors *
                                          hidden_outputs * (1.0 - hidden_outputs)), numpy.transpose
                                         (inputs)))/batch_size+lamda/batch_size*self.wih
        # print("---------------------改变后的  who----------------------------")
        # print(self.who[0])
        # pause=input()
